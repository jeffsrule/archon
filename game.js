...
//
// Maps unit types to their movement audio file paths.
// The audio manager preloads and loops these while the unit is actively moving.
// To change a unit's movement sound, update only the path here.
//
const MOVE_SFX_BY_UNIT = {
    'Knight':    'assets/audio/Knight Walk.m4a',
    'Goblin':    'assets/audio/Knight Walk.m4a',
    'Archer':    'assets/audio/Knight Walk.m4a',
    'Unicorn':   'assets/audio/Unicorn Walk.m4a',
    'Golem':     'assets/audio/Golem Walk.m4a',
    'Basilisk':  'assets/audio/Knight Walk.m4a',
    'Dragon':    'assets/audio/Dragon Walk.m4a',
    'Troll':     'assets/audio/Troll Walk.m4a',
    'Banshee':   'assets/audio/Banshee Walk.m4a',
    'Manticore': 'assets/audio/Manticore Walk.m4a',
    'Valkyrie':      'assets/audio/Fly Sound.m4a',
...
class ArchonGame {
    constructor() {
        ...
        this.movementAudioByPath = {};
        this.movementAudioCache = {};
        this.activeMovementAudio = null;
        this.activeMovementAudioType = null;
        const preloadMovePath = (p) => {
            if (!this.movementAudioByPath[p]) {
                const a = new Audio(p);
                a.loop = true;
                a.volume = 0.5;
                a.preload = 'auto';
                this.movementAudioByPath[p] = a;
            }
            return this.movementAudioByPath[p];
        };
        for (const [unitType, entry] of Object.entries(MOVE_SFX_BY_UNIT)) {
            if (typeof entry === 'string') {
                this.movementAudioCache[unitType] = preloadMovePath(entry);
            } else {
                const stratAudio = preloadMovePath(entry.strategy);
                stratAudio.loop = false;
                this.movementAudioCache[unitType] = {
                    strategy: stratAudio,
                    combat: preloadMovePath(entry.combat)
                };
            }
        }

        this.oneShotCtx = null;
        this.oneShotBuffers = {};
        this.oneShotPool = {};
        this.oneShotVolume = 0.5;
        this.lastTurnSoundPlayedFor = null;
        this.initOneShotSFX();

        this.winThemeAudio = new Audio('assets/audio/Splash Screen Audio.m4a');
        this.winThemeAudio.loop = true;
        this.winThemeAudio.volume = 0.5;
        this.winThemeAudio.preload = 'auto';
        this.winThemePlaying = false;
    ...
    startMovementAudio(unitType) {
        if (!unitType) return;
        const cached = this.movementAudioCache[unitType];
        if (!cached) return;
        let audio;
        if (cached instanceof Audio) {
            audio = cached;
        } else {
            const screen = this.gameState === 'COMBAT' ? 'combat' : 'strategy';
            audio = cached[screen];
        }
        if (!audio) return;
        if (this.activeMovementAudio === audio && !audio.paused) return;
        this.stopMovementAudio();
        this.activeMovementAudio = audio;
        this.activeMovementAudioType = unitType;
        audio.currentTime = 0;
        audio.play().catch(() => {});
    }

    stopMovementAudio() {
        if (!this.activeMovementAudio) return;
        this.activeMovementAudio.pause();
        this.activeMovementAudio.currentTime = 0;
        this.activeMovementAudio = null;
        this.activeMovementAudioType = null;
    }
    ...
    initOneShotSFX() {
        ...
        if (AudioCtx) {
            try {
                this.oneShotCtx = new AudioCtx();
            } catch (e) {
                this.oneShotCtx = null;
            }
        }
        for (const [key, path] of Object.entries(ONE_SHOT_SFX)) {
            if (this.oneShotCtx) {
                fetch(path)
                    .then(r => r.arrayBuffer())
                    .then(buf => this.oneShotCtx.decodeAudioData(buf))
                    .then(decoded => { this.oneShotBuffers[key] = decoded; })
                    .catch(() => {});
            }
            const pool = [];
            for (let i = 0; i < 4; i++) {
                ...
                a.preload = 'auto';
                a.volume = this.oneShotVolume;
                pool.push(a);
            }
            this.oneShotPool[key] = pool;
        }
    }

    unlockAudio() {
        if (this.oneShotCtx && this.oneShotCtx.state === 'suspended') {
            this.oneShotCtx.resume().catch(() => {});
        }
    }

    playOneShot(key, opts) {
        const vol = opts?.volume ?? this.oneShotVolume;
        if (this.oneShotCtx && this.oneShotCtx.state === 'running' && this.oneShotBuffers[key]) {
            const src = this.oneShotCtx.createBufferSource();
            src.buffer = this.oneShotBuffers[key];
            const gain = this.oneShotCtx.createGain();
            gain.gain.value = vol;
            src.connect(gain);
            gain.connect(this.oneShotCtx.destination);
            src.start(0);
            return;
        }
        const pool = this.oneShotPool[key];
        if (!pool || pool.length === 0) return;
        let elem = pool.find(a => a.paused || a.ended);
        if (!elem) {
            elem = pool[0].cloneNode(true);
            elem.volume = vol;
            pool.push(elem);
        }
        elem.volume = vol;
    ...
    playTurnChangeSound() {
        ...
        if (this.lastTurnSoundPlayedFor === this.currentSide) return;
        this.lastTurnSoundPlayedFor = this.currentSide;
        clearTimeout(this.turnChangeSoundTimer);
        this.turnChangeSoundTimer = setTimeout(() => {
            this.playOneShot('turnChange');
        }, 250);
    }

    startWinTheme() {
        if (this.winThemePlaying) return;
        this.winThemePlaying = true;
        this.winThemeAudio.currentTime = 0;
        this.winThemeAudio.play().catch(() => {});
    }

    stopWinTheme() {
        if (!this.winThemePlaying) return;
        this.winThemePlaying = false;
        this.winThemeAudio.pause();
        this.winThemeAudio.currentTime = 0;
    }
    ...
    isMage(piece) {
        return piece && (piece.type === 'Wizard' || piece.type === 'Sorceress');
    }

    canOpenSpellMenuFor(piece) {
        return piece
            && this.isMage(piece)
    ...
    updateStrategy(deltaTime) {
        ...
        if (this.spellMenu.message !== null) {
        ...
        if (this.spellMenu.active) {
            this.spellMenu.scrollCooldown = Math.max(0, this.spellMenu.scrollCooldown - deltaTime);
            ...
            if (this.keys[upKey]) {
                ...
                this.spellMenu.selectedIndex = (this.spellMenu.selectedIndex - 1 + spellCount) % spellCount;
            }
            if (this.keys[downKey]) {
                this.keys[downKey] = false;
                this.spellMenu.selectedIndex = (this.spellMenu.selectedIndex + 1) % spellCount;
            }

            if (gp) {
                if (this.spellMenu.scrollCooldown <= 0) {
                    const ax1 = gp.axes[1] ?? 0;
                    if (ax1 < -0.5) {
                        this.spellMenu.selectedIndex = (this.spellMenu.selectedIndex - 1 + spellCount) % spellCount;
                        this.spellMenu.scrollCooldown = 0.15;
                    } else if (ax1 > 0.5) {
                        this.spellMenu.selectedIndex = (this.spellMenu.selectedIndex + 1) % spellCount;
                        this.spellMenu.scrollCooldown = 0.15;
                    }
        ...
        if (this.keys[fireKey]) {
            this.keys[fireKey] = false;
            const now = performance.now();
            if (now - this.lastFirePressTime < this.fireDoublePressWindow && this.canOpenSpellMenuFor(this.selectedPiece)) {
                this.openSpellMenu(this.selectedPiece);
                this.lastFirePressTime = 0;
            } else {
                this.lastFirePressTime = now;
            }
        }

        if (gp) {
            const bDown = gp.buttons[1]?.pressed ?? false;
            if (bDown && !this.gamepadBPressed && this.canOpenSpellMenuFor(this.selectedPiece)) {
                this.openSpellMenu(this.selectedPiece);
            }
            this.gamepadBPressed = bDown;
        }
    ...
    updateCombat(deltaTime) {
        ...
        {
            let combatMovingType = null;
            if (lightActor?.isMoving && lightPiece) {
                combatMovingType = lightPiece.type;
            }
            if (darkActor?.isMoving && darkPiece) {
                combatMovingType = combatMovingType ?? darkPiece.type;
            }
            if (combatMovingType && this.movementAudioCache[combatMovingType]) {
                this.startMovementAudio(combatMovingType);
            } else if (this.activeMovementAudio && this.gameState === 'COMBAT') {
                this.stopMovementAudio();
            }
        }
    ...
    drawTestPattern() {
        ...
        if (this.spellState.activeSpell) {
            ...
            const spellName = this.spellMenu.spells[this.spellMenu.selectedIndex] ?? '';
            this.ctx.fillText(spellName, smX, smY + 24);
            this.ctx.fillText(spellName, smX + 1, smY + 24);
            this.ctx.restore();
    ...
    tryStartTeleportMove(piece, destX, destY) {
        ...
        if (this.movementAudioCache[piece.type]) {
            this.startMovementAudio(piece.type);
            clearTimeout(this.teleportSoundTimer);
            this.teleportSoundTimer = setTimeout(() => {
